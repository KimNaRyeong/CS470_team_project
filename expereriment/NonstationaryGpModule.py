...
class NonstationaryGpModule(pl.LightningModule):
  ...
  def condition_level(self, gaussian_covs, level, sparse_coords_norm, sparse_depth, mean_depth, test_size):
    ...
    # sparse_vars = sparse_var_level * torch.ones_like(sparse_depth).squeeze()
    # sparse_vars = sparse_vars.view(sparse_vars.size(0), -1)
    # sparse_vars = sparse_vars.unsqueeze(-1).expand(sparse_vars.size(0), sparse_vars.size(1), sparse_vars.size(1))
  ...
  def prep_train_level(self, level, num_levels, gt_depth, gaussian_covs):
    ...
    # print(gt_depth.dim())
    if gt_depth.dim() == 5:
      gt_depth = gt_depth.view(b, -1, gt_depth.shape[3], gt_depth.shape[4])
  ...
  def training_step(self, batch, batch_idx):
    ...
    print(f"Training loss: {outputs['final_level_nll']}")
  ...
  def training_epoch_end(self, outputs):
    if len(outputs) > 0:
      ...
      print(f"Training epoch end - Loss: {loss}")  # Debug print
  ...
  def validation_step(self, batch, batch_idx):
    ...
    outputs = self.get_loss(batch, mode="val")
    # return loss

    ###########################################################
    # Implement for accuracy
    ###########################################################
    if outputs is None:
      return None
    ...
    # Prepare coordinates and depth
    test_size = gt_depth.shape[-2:]
    sparse_coords_norm, sparse_log_depth, mean_depth = self.prep_sparse_coords(gt_depth, gaussian_covs)

    if sparse_coords_norm is None:
        return None

    pred_log_depths, pred_vars = self.condition(
        gaussian_covs, sparse_coords_norm, sparse_log_depth, mean_depth, test_size
    )

    rmse = self.calculate_rmse(pred_log_depths[-1], gt_depth)

    return {"final_level_nll": outputs["final_level_nll"], "params_extrema": outputs["params_extrema"], "rmse": rmse}
  ...
  def calculate_rmse(self, predicted, ground_truth):
    # # print(predicted)
    # print("================")
    # print(ground_truth)
    # for value in np.nditer(ground_truth):
      # self.depth_set.add(ground_truth)
    ...
    #       for m in k:
    #         for n in m:
    #           self.depth_set.add(n.item())
    return torch.sqrt(torch.mean((predicted - ground_truth) ** 2))
  
  def prep_sparse_coords(self, gt_depth, gaussian_covs):
    b = gt_depth.shape[0]
    device = gt_depth.device
    num_levels = len(gaussian_covs)

    sparse_coords_norm = None
    sparse_log_depth = None
    mean_depth = None
    ...
    with torch.no_grad():
        ...
        depth = resize_depth(gt_depth, mode="bilinear", size=gaussian_covs[0].shape[-2:])
        coord_train, depth_train, batch_train = sample_coords(depth, gaussian_covs, self.max_train_samples, mode="uniform")
        if coord_train is not None:
            batch_train = torch.arange(b, device=device).unsqueeze(1).repeat(1, coord_train.shape[1])
            sparse_coords_norm = normalize_coordinates(coord_train, gaussian_covs[0].shape[-2:])
            sparse_log_depth = torch.log1p(depth_train)
            mean_depth = torch.mean(sparse_log_depth, [1, 2], keepdim=True)

    return sparse_coords_norm, sparse_log_depth, mean_depth
